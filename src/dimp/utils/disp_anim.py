...
from dimp.utils.disp import gen_arrow_head_marker, init_matplotlib
...
class Animation:
    def __init__(self, data, goals, obstacles, ax, dt) -> None:
        init_matplotlib()
        self.textsize = plt.rcParams.get('xtick.labelsize')
        ...
        self.coloured_trajectory = False
            ...
            for c in range(len(self.data[frame])):
                for j, s_c_j in enumerate(state[c]):
                    ...
                    if self.coloured_trajectory:
                        self.artists.past_trajectory[cnt] = plot_colour_line(x, y)
                    else:
...
def display_animation(
    ...
    Args:
        s_history (_type_): _description_
        goals (_type_): _description_
        obstacles (_type_): _description_
        dt (float): _description_
        method (str, optional): _description_. Defaults to 'plot'.
        x_lim (list, optional): _description_. Defaults to [-20.0, 20.0].
        y_lim (list, optional): _description_. Defaults to [-20.0, 20.0].
        video_name (str, optional): _description_. Defaults to 'video.mp4'.
        flags (MultiRobotArtistFlags, optional): _description_. Defaults to MultiRobotArtistFlags().
    ...
    elif method == 'save' or method == 'html':
        ani = FuncAnimation(
            fig=fig,
            func=anim.update,
            init_func=anim.init,
